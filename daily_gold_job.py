...
# Nếu cosine >= SIM_THRESHOLD_LOC (0.80) hoặc fuzz.token_set_ratio >= 90, coi là tương đồng.
...
def merge_duplicate_types_and_update_fact(c):
    """(Tuỳ chọn) Gộp TYPE tương đồng về 1 ID và cập nhật FACT.TYPE_ID."""
    df = pd.read_sql('SELECT ID AS TYPE_ID, TYPE_NAME, PURITY, CATEGORY FROM GOLD_TYPE_DIMENSION', c)
    values = df["TYPE_NAME"].astype(str).str.lower().fillna("").tolist()
    groups = build_similarity_groups(values, SIM_THRESHOLD_TYPE)
    mapping = {}
    for grp in groups:
        ids = df.iloc[grp]["TYPE_ID"].tolist()
        canon = min(ids)
        for idx in grp:
            tid = int(df.iloc[idx]["TYPE_ID"])
            if tid != canon:
                mapping[tid] = canon
    ...
    if mapping:
        with c.cursor() as cur:
            for old_id, new_id in mapping.items():
                cur.execute("UPDATE GOLD_PRICE_FACT SET TYPE_ID = :new WHERE TYPE_ID = :old", {"new": new_id, "old": old_id})
        c.commit()
        print(f"🔁 Gộp TYPE và cập nhật FACT: {mapping}")
    else:
        print("ℹ️ Không có TYPE tương đồng cần gộp.")
